...
module wijmo.grid.grouppanel {
    ...
    export class GroupPanel extends Control {
        ...
        _gds: collections.ObservableArray;          // groupDescriptions being edited
        ...
        set grid(value: FlexGrid) {
            value = <FlexGrid>asType(value, FlexGrid, true);
            if (value != this._g) {
        ...
        _addGroup(col: Column, e: MouseEvent) {
            ...
            gds.deferUpdate(() => {
                var gd = new collections.PropertyGroupDescription(col.binding);
                gds.insert(index, gd);
            });
        ...
        _click(e) {
            ...
            if (hasClass(marker, 'wj-cell')) {
                ...
                if (remove) { // remove group
                    ...
                    var sd = new collections.SortDescription(gd.propertyName, asc);
