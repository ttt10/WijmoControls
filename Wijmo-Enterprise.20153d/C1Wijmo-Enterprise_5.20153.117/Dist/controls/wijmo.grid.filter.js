                ...
                Object.defineProperty(ConditionFilter.prototype, "and", {
                    ...
                    */
                    ...
                    set: function (value) {
                        ...
                        this._bnd = this._col && this._col.binding ? new wijmo.Binding(this._col.binding) : null;
                    },
                    enumerable: true,
                    configurable: true
                });
            ...
            var FlexGridFilter = (function () {
                ...
                */
                function FlexGridFilter(grid) {
                    ...
                    */
                    ...
                    // create glyph filter element
                    this._filterGlyph = wijmo.createElement('<div ' + FlexGridFilter._WJA_FILTER + ' style ="float:' + (grid._rtl ? 'left' : 'right') + ';cursor:pointer;padding:0px 4px">' + '<span class="wj-glyph-filter"></span>' + '</div>');
                        ...
                        for (var i = 0; i < def.filters.length; i++) {
                            ...
                            if (cf) {
                                switch (cfs.type) {
                                    case 'condition':
                                        var cfc = cf.conditionFilter;
                                        cfc.condition1.value = col.dataType == 4 /* Date */ ? wijmo.changeType(cfs.condition1.value, col.dataType, null) : cfs.condition1.value;
                                        cfc.condition1.operator = cfs.condition1.operator;
                                        cfc.and = cfs.and;
                                        cfc.condition2.value = col.dataType == 4 /* Date */ ? wijmo.changeType(cfs.condition2.value, col.dataType, null) : cfs.condition2.value;
                ...
                */
                FlexGridFilter.prototype.editColumnFilter = function (col, ht) {
                    ...
                    var ch = this._grid.columnHeaders, r = ht ? ht.row : ch.rows.length - 1, c = ht ? ht.col : col.index, rc = ch.getCellBoundingRect(r, c), hdrCell = document.elementFromPoint(rc.left + rc.width / 2, rc.top + rc.height / 2);
                ...
                */
                ...
                FlexGridFilter.prototype._formatItem = function (sender, e) {
                    ...
                    if (e.panel.cellType == 2 /* ColumnHeader */) {
                        // get column, binding column
                        var g = this._grid, rng = g.getMergedRange(e.panel, e.row, e.col) || new _grid.CellRange(e.row, e.col), col = g.columns[rng.col], bcol = g._getBindingColumn(e.panel, e.row, col);

                        // check that the row is valid for the filter icon
                        if (rng.row2 == e.panel.rows.length - 1 || col != bcol) {
                            // get the filter for this column
                            var cf = this.getColumnFilter(bcol, this.defaultFilterType != 0 /* None */);
                            ...
                            if (cf && cf.filterType != 0 /* None */) {
                                // handle RTL
                                var rtl = g.hostElement && getComputedStyle(g.hostElement).direction == 'rtl';

                                // show filter glyph for this column
                                if (this._showIcons) {
                                    var cell = (e.cell.querySelector('div') || e.cell), glyph = this._filterGlyph.cloneNode(true);
                ...
                FlexGridFilter.prototype._mouseDown = function (e) {
                    var _this = this;
                    if (wijmo.closest(e.target, '[' + FlexGridFilter._WJA_FILTER + ']')) {
                        var g = this._grid, ht = g.hitTest(e);
