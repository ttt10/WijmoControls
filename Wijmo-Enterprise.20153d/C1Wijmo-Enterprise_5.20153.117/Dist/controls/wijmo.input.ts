...
module wijmo.input {
    ...
    export class Calendar extends Control {
        ...
        private _addDays(days: number): Date {
            if (!this.value) return this.value; // TFS 141664
            ...
            this.addEventListener(host, 'onmousewheel' in document ? 'mousewheel' : 'DOMMouseScroll', (e: MouseWheelEvent) => {
                if (host.scrollHeight > host.offsetHeight) {
                    if ((e.wheelDelta > 0 && host.scrollTop == 0) ||
                        (e.wheelDelta < 0 && host.scrollTop + host.offsetHeight >= host.scrollHeight)) {
                        e.preventDefault();
                        e.stopPropagation();
                    }
                }
            ...
            if (!this.isVisible) {
                ...
                if (this.onShowing(e)) {
                    ...
                    setTimeout(() => {
                        ...
                        if (!this.isTouching) {
                            var inputs = this.hostElement.querySelectorAll('input');
    ...
    export class InputDate extends DropDown {
        ...
        _createDropDown() {
            ...
            this.addEventListener(this._dropDown, 'mouseup', () => {
                var value = this._calendar.value;
    ...
    export class InputTime extends ComboBox {
        ...
        _updateItems() {
            ...
            if (isNumber(this.step) && this.step > 0) {
                for (var dt = min; dt <= max; dt = DateTime.addMinutes(dt, this.step)) {
...
module wijmo.input {
    ...
    export class InputDateTime extends InputDate {
        ...
        constructor(element: any, options?) {
            ...
            this.addEventListener(btnTm, 'mousedown', (e: MouseEvent) => {
        ...
        _updateBtn() {
            super._updateBtn();
            var btnTm = <HTMLElement>this.hostElement.querySelector('[wj-part="btn-tm"]');
            if (btnTm) {
...
module wijmo.input {
    ...
    export class InputNumber extends Control {
        ...
        set value(value: number) {
            if (value != this._value) {
                ...
                if (value == null) {
                    ...
                    value = this._clamp(value);
                    var text = Globalize.format(value, this.format, false, true);
                    this._setText(text);
                }
            }
        }
        ...
        onLostFocus(e?: EventArgs) {
            var text = Globalize.format(this.value, this.format);
            this._setText(text);
            super.onLostFocus(e);
        }
        ...
        refresh(fullUpdate?: boolean) {
            this._updateSymbols();
            var text = Globalize.format(this.value, this.format, false, true);
            this._setText(text);
        }
        ...
        private _updateSymbols() {
            var nf = culture.Globalize.numberFormat;
            this._decChar = nf['.'] || '.';
        ...
        private _setText(text: string) {
            ...
            // get formatted value
            var fval = Globalize.format(value, fmt, false, true);
            if (fmt == 'n' || fmt[0].toLowerCase() == 'g') { // allow trailing '.', '.0' // TFS 140302
        ...
        private _input(e) {
            ...
            setTimeout(() => {
                ...
                var tbx = this._tbx,
                    ...
                    dec = text ? text.indexOf(this._decChar): -1;
                ...
                if (this.containsFocus()) {
                    ...
                    if (text) {
                        if ((sel <= dec && newDec > -1) || (dec < 0 && newDec < 0)) { // cursor was on the left of the decimal
        ...
        private _clickSpinner(e: MouseEvent) {
            if (this.value != null && this.step != null) {
                ...
                var value = this.value + this.step * (contains(this._btnUp, e.target) ? +1 : -1),
