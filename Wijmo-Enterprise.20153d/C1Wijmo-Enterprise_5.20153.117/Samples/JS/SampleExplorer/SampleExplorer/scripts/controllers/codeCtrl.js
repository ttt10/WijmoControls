...
.controller('codeCtrl', function ($scope, $http, $sce, $routeParams, SamplesService, SourceCodeService) {
    ...
    */
    ...
    function addIcons(panel, r, c, cell) {
        ...
        switch ($scope.fileTree.flex.rows[r].dataItem.type) {
            case 'file':
                cell.innerHTML = '<span class="glyphicon glyphicon-file tree-icon"></span>' + cell.innerHTML;
                break;
            case 'dir':
                var icon = $scope.fileTree.flex.rows[r].isCollapsed ? 'close' : 'open';
                cell.innerHTML = '<span wj-collapse class="glyphicon glyphicon-folder-' + icon + ' tree-icon"></span>' + $scope.fileTree.flex.rows[r].dataItem.DirTitle;
                break;
        }
    }
