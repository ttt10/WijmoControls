...
app.controller('basicCtrl', function appCtrl($scope, $compile, dataSvc) {
    ...
    $scope.saveColumnLayout = function () {
        if (localStorage) {
            ...
            console.log('**LAYOUT: ' + flex.columnLayout);
        }
    }
    ...
    // ** popup editor: 
    // start editing the current item when the detail dialog gets the focus
    $('#dlgDetail').on('shown.bs.modal', function (e) {
        $scope.ctx.flex.collectionView.editItem($scope.current);
    });
