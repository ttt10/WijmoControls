    ...
    */
    var Portfolio = (function () {
        ...
        Portfolio.prototype.getCompany = function (symbol) {
            var self = this, protocol = 'https', c = Portfolio._companyCache[symbol], url = '://www.quandl.com/api/v3/datasets/WIKI/', token = 'rX6NsszGKZp32RUbA7SR', startDate = wijmo.Globalize.formatDate(new Date("01/01/2008"), "yyyy-MM-dd"), endDate = wijmo.Globalize.formatDate(new Date(), "yyyy-MM-dd");
            ...
            if (c == null) {
                ...
                $.get(protocol + url + symbol, {
                    ...
                    c.name = result.dataset.name.substring(0, result.dataset.name.indexOf('('));
                    self.viewChanged();

                    //console.log('got ' + c.symbol + 'name: ' + c.name);
                    // store prices
                    var lines = result.dataset.data;
                    for (var i = 0; i < lines.length; i++) {
                        var item = lines[i];
                        var date = new Date($.trim(item[0]).replace(/-/g, '/')), price = parseFloat(item[1]);
