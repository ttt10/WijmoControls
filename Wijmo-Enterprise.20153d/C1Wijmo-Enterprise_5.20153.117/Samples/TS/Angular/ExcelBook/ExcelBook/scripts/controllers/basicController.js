...
angular.module('app').controller('basicController', function ($scope, dataService) {
	...
	$scope.applyCellStyle = function (className, cancelCellStyle) {
		if (cancelCellStyle) {
			...
			if (className) {
				...
				$scope.ctx.flexSheet.applyCellsStyle({ className: className }, undefined, true);
			} else if (appliedClass) {
				$scope.ctx.flexSheet.applyCellsStyle({ className: appliedClass });
				appliedClass = '';
	...
	$scope.applyCellVerticalAlign = function (verticalAlign) {
		$scope.ctx.flexSheet.applyCellsStyle({ verticalAlign: verticalAlign });
	};
	...
	$scope.pendingCellEditAction = function () {
		pendingAction = new wijmo.grid.sheet._EditAction($scope.ctx.flexSheet);
	}
	...
	$scope.undo = function () {
		$scope.ctx.flexSheet.undo();
	};

	// Excutes redo command.
	$scope.redo = function () {
		$scope.ctx.flexSheet.redo();
	};
